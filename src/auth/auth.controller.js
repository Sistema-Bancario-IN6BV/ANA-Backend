...
import { findAllElderly } from '../../helpers/user-db.js';
...
/**
 * GET /api/v1/auth/elderly
 * Obtener todos los usuarios con rol ELDERLY_ROLE
 * Solo accesible para ADMIN y CAREGIVER
 */
export const getElderly = [
  validateJWT,
  asyncHandler(async (req, res) => {
    try {
      const elderly = await findAllElderly();

      return res.status(200).json({
        success: true,
        message: 'Adultos mayores obtenidos correctamente',
        data: elderly,
      });
    } catch (error) {
      ...
      return res.status(500).json({
        success: false,
        message: 'Error al obtener adultos mayores',
        error: error.message,
      });
    }
  }),
